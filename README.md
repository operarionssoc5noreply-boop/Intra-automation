...
Lightweight bot server for SeaTalk. Google Apps Script watches cell `AD1` in your sheet, and when that value changes it calls this service. The service then renders the report range as an image, reads `AE2` for the FMS update timestamp, and sends one interactive message card to the SeaTalk group webhook.
