...
def load_config() -> Config:
    ...
    config = Config(
        ...
        capture_delay_seconds=int(
            get_setting(env_file_values, "capture_delay_seconds", "BOT_CAPTURE_DELAY_SECONDS", "15")
        ),
            ...
            "elements": [
                {
                    "element_type": "title",
                    "title": {
                        "text": f"SOC 5 IntraDay Update as of {timestamp}",
                    },
                },
                {
...
class SeatalkBotService:
    ...
    def fetch_fms_update_value(self, trigger_metadata: dict[str, Any] | None = None) -> str:
        ...
        if trigger_metadata:
            fallback_value = format_sheet_datetime_value(trigger_metadata.get("current_value"))
            if fallback_value:
                LOGGER.warning("Using trigger metadata current_value as fallback for %s.", FMS_UPDATE_CELL_A1)
                return fallback_value

        LOGGER.warning("Cell %s was blank and no trigger metadata fallback was available.", FMS_UPDATE_CELL_A1)
        return "-"
    ...
    def build_message_payload(
        ...
        description_value = self.fetch_fms_update_value(trigger_metadata)
